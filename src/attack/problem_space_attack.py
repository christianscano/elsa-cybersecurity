...
def _apply_manipulations(i, manipulator: Manipulator, pop, manipulation_space):
...
class ProblemSpaceAttack:
    """Class for performing problem-space attacks on APK files."""

    # TODO: This should also take an optmizer object
    def __init__(
        self,
    ...
    def fitness(
        ...
        """
        Calculate the fitness of a list of individuals.
        ...
        pop : list of np.ndarray
            List of arrays containing the individuals (i.e., the manipulations to be
            applied to the malware sample).
        manipulation_space : ManipulationSpace
        ...
        """
        self.logger.debug("Applying manipulations to %d samples", len(population))
        updated_pop = [
            ind
            for ind in parfor2(
            ...
            if ind.apk_path is not None and Path(ind.apk_path).is_file()
        ]

        if not updated_pop:
            self.logger.debug("All the manipulations failed")
        else:
            self.logger.debug("Computing fitness for %d samples", len(updated_pop))
    ...
    def crossover(self, ind1, ind2):
        """
        Execute a crossover between the manipulations contained in the two
    ...
    def _run_attack(self, malware_sample: str) -> tuple[int, float, str]:
        ...
        try:
            ...
            population, toolbox, manipulation_space = self._init_attack(
                malware_sample,
                manipulator
            )

            # Evaluate the entire population
            population = self.fitness(manipulator, population, manipulation_space)
            ...
            while g < self._n_iterations:
                ...
                offspring = toolbox.select(population, self._n_candidates)
                ...
                # Update adv sample in the first iteration and if fitness is improved
                if g == 0 or (
                    len(last_n_best_fits) > 1 and best_fitness != last_n_best_fits[-2]
                ):
                    label = population[best_idx].label
                    score = population[best_idx].score
                    adv_apk_path = population[best_idx].apk_path
                    self.logger.debug(f"Generation {g + 1} - score {best_fitness}")
                    # early stop
                    if label == 0:
                        self.logger.info("Attack finished - score %s", best_fitness)
                        return
                else:
            ...
            self.logger.error("Error with sample %s: %s", malware_sample, e)  # noqa: TRY400
        finally:
            if manipulator:
            ...
            if population:
                for ind in population:
                    if ind.apk_path == adv_apk_path:
                        adv_apk_path = Path(ind.apk_path).parent / \
                                       f"adv_{Path(malware_sample).name}"
                        Path(ind.apk_path).rename(adv_apk_path)
                    elif Path(ind.apk_path).exists():
                        Path(ind.apk_path).unlink()

        return label, score, adv_apk_path

    def _init_attack(self, malware_sample: str, manipulator: Manipulator) -> tuple:
        """
        ...
        def init_population(pcls, ind_init):
            """
            Initialize the manipulation vectors from the manipulation space
    ...
    def _build_manipulation_space(
        ...
        error_free_injections = manipulator.get_error_free_manipulations(
            manipulation_space.get_all_injections(),
            self._n_jobs
        )
        ...
        error_free_manipulations = Manipulations(
            error_free_injections.inject,
            error_free_obfuscations.obfuscate
        )

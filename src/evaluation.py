...
Functions
---------
evaluate(classifier: BaseModel, config)
    Evaluates the classifier on the goodware and adversarial test set dataset.
"""
...
from .attack import ProblemSpaceAttack
...
def evaluate(classifier: BaseModel, config) -> list[dict]:
    ...
    #
    ts_fp_check = sorted(
        [Path(ts_fp_check_dir) / apk for apk in os.listdir(ts_fp_check_dir)],
    ...
    #
    ts_adv = sorted(
        [Path(ts_adv_dir) / apk for apk in os.listdir(ts_adv_dir)],
        key=os.path.getctime,
    )
